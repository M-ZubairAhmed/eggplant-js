...
function startInitialPaint(element, container) {
  let domNode
    ...
    element.props.children.forEach((child) => {
      requestIdleCallback(
        () => {
          ...
          startInitialPaint(child, domNode)
        },
        { timeout: 1000 }
      )
    })
...
//   startInitialPaint(elem, container)
...
function render(element, passedInContainer) {
  ...
  startInitialPaint(element(), passedInContainer)
}
